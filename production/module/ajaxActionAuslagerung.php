...
if(isset($_POST["action"])) {
    ...
    if($_POST['action'] == 'FETCHONE'){
        ...
        $stmt = $connection->prepare("SELECT b.bestellung_nr as `bestellung_nr`,DATE_FORMAT(b.datum, '%d.%m.%Y') as `datum`, p.name as `produkt_name`, bp.menge as `menge`,bp.bestellung_id as `bestellung_id`, bp.produkt_id as `produkt_id`,bp.gelagert as `gelagert` FROM bestellung_position bp, bestellung b , produkt p WHERE bp.bestellung_id = b.bestellung_id AND bp.produkt_id = p.produkt_id AND bp.bestellung_id = ? AND bp.produkt_id = ?");
    ...
    $query = "SELECT sum(pf.ausgelagerte_menge) as `sum_ausgelagerte_menge`,b.bestellung_nr as `bestellung_nr`,
DATE_FORMAT(b.datum, '%d.%m.%Y') as `datum`,
p.name as `produkt_name`, bp.menge as `menge`,
bp.bestellung_id as `bestellung_id`,
bp.produkt_id as `produkt_id`,
(CASE 
	WHEN sum(pf.ausgelagerte_menge) = bp.menge THEN 'komplett bearbeitet' 
    WHEN sum(pf.ausgelagerte_menge) < bp.menge THEN 'teilweise bearbeitet'
    ELSE 'noch nicht bearbietet'
END) as `status`
FROM bestellung_position bp JOIN bestellung b ON bp.bestellung_id = b.bestellung_id 
JOIN produkt p ON bp.produkt_id = p.produkt_id 
LEFT JOIN position_fach_bestellung pf ON bp.bestellung_id = pf.bestellung_id 
AND bp.produkt_id = pf.produkt_id 
WHERE bp.gelagert = 0
GROUP BY bp.bestellung_id, bp.produkt_id  ";


    if(!empty($_POST["searchPhrase"]))
    {
        $query .= 'HAVING bestellung_nr LIKE "%'.$_POST["searchPhrase"].'%" ';
    ...
    {
        $query .= 'ORDER BY bestellung_nr DESC ';
    }
    if($order_by != '')
    {
    ...
    $query1 = "SELECT * FROM bestellung_position WHERE gelagert = 0";
