...
WHERE r.lagerregal_id = f.lagerregal_id AND f.bestand < f.max_kapazitaet AND f.fachregal_id = ?");
    ...
    if ($_POST['action'] == "Speichern") {
        ...
        $label_farbe = $_POST['label_farbe'];


        $prepared = $connection->prepare("INSERT INTO `lagerregal`(`name`,`nummer`,`barcode`, `fachanzahl`,`label_farbe`) VALUES (?,?,?,?,?);");

        if ($prepared == false)
            die("Secured1");

        $result = $prepared->bind_param("sisis", $name,$nummer, $barcode, $fachanzahl,$label_farbe);
        if ($result == false)
            die("Secured2");
        ...
        for($i = 0; $i < $fachanzahl; $i++) {
            ...
            $result2 = $prepared2->bind_param("ssii", $fachname, $barcode, $kapazitaet,$lagerregal_id);
            if ($result2 == false)
                die("Secured22");
    ...
    //wareHouse.pphp
    if($_POST['action'] == 'REMOVE_REGAL'){

        $lagerregal_id = $_POST['lagerregal_id'];


        $prepared = $connection->prepare("DELETE FROM `lagerregal` WHERE `lagerregal_id`= ?");

        if ($prepared == false)
            die("Secured1");

        $result = $prepared->bind_param("i", $lagerregal_id);
        if ($result == false)
            die("Secured2");
        ...
        //delete Fächern (Cascading)

        $prepared3 = $connection->prepare("DELETE FROM `fachregal` WHERE `lagerregal_id`= ?");

        if ($prepared3 == false)
            die("Secured11");

        $result3 = $prepared3->bind_param("i", $lagerregal_id);
        if ($result3 == false)
            die("Secured22");
    ...
    // Reservation.php FETCHONE
    if($_POST['action'] == 'FETCHONE'){

        $output = Array();
        $fachregal_id = $_POST['id'];
//nur die frei Plätze die
        $stmt = $connection->prepare("SELECT f.name as `fachregal`,f.fachregal_id as `fachregal_id`,f.max_kapazitaet as `max_kapazitaet`, f.bestand as `bestand` 
FROM lagerregal r, fachregal f
WHERE r.lagerregal_id = f.lagerregal_id AND f.bestand < f.max_kapazitaet AND f.fachregal_id = ?");
        $stmt->bind_param("i",$fachregal_id);
    ...
    if($_POST['action'] == 'FETCHONE_BESTELLUNG'){
...
//nur die frei Plätze die
        $stmt = $connection->prepare("SELECT f.name as `fachregal`,f.fachregal_id as `fachregal_id`,f.max_kapazitaet as `max_kapazitaet`, f.bestand as `bestand` 
FROM lagerregal r, fachregal f
WHERE r.lagerregal_id = f.lagerregal_id AND f.bestand > 0 AND f.fachregal_id = ?");
        $stmt->bind_param("i",$fachregal_id);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    if($_POST['action'] == 'FETCHALL')
    {
        ...
        $stmt = $connection->prepare("SELECT l.lagerregal_id,l.name,l.nummer,l.barcode, l.fachanzahl,l.label_farbe,
(SELECT f.max_kapazitaet FROM fachregal f WHERE l.lagerregal_id = f.lagerregal_id LIMIT 1) as  `kapazitaet` 
FROM lagerregal l");
    ...
    if($_POST['action'] == 'GETBARCODE'){
        ...
        $stmt = $connection->prepare("SELECT f.lagerregal_id as `lagerregal_id`, r.name as `regalname`, r.nummer as `regalnummer`, fachanzahl ,f.name as `fachregal`,f.fachregal_id as `fachregal_id`, f.barcode as `fachbarcode`, f.max_kapazitaet as `kapazitaet` FROM lagerregal r, fachregal f
