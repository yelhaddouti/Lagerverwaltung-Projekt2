...
if(isset($_POST["action"])) {
    ...
    if($_POST['action'] == 'FETCHONE'){
        ...
        $stmt = $connection->prepare("SELECT p.produkt_id as `produkt_id`,p.name as `produkt_name`,lp.menge as `menge`, li.lieferung_id as `lieferung_id`, li.lieferung_nr as `lieferung_nr`, DATE_FORMAT(li.datum, '%d.%m.%Y') as `datum`, l.name as `name`, li.lieferant_id as `lieferant_id` 
FROM lieferung li, lieferant l ,lieferung_position lp,produkt p WHERE li.lieferant_id = l.lieferant_id AND lp.produkt_id = p.produkt_id AND lp.lieferung_id = li.lieferung_id AND li.lieferung_id = ?");
    ...
    $query .= "SELECT li.lieferung_id as `lieferung_id`, li.lieferung_nr as `lieferung_nr`, DATE_FORMAT(li.datum, '%d.%m.%Y') as `datum`, l.name as `name` 
            FROM lieferung li, lieferant l WHERE li.lieferant_id = l.lieferant_id ";

    if(!empty($_POST["searchPhrase"]))
    {
        $query .= 'AND (li.lieferung_id LIKE "%'.$_POST["searchPhrase"].'%" ';
        $query .= 'OR li.lieferung_nr LIKE "%'.$_POST["searchPhrase"].'%" ';
        $query .= 'OR DATE_FORMAT(li.datum, "%d.%m.%Y") LIKE "%'.$_POST["searchPhrase"].'%" ';
        $query .= 'OR l.name LIKE "%'.$_POST["searchPhrase"].'%" ) ';
    ...
    {
        $query .= 'ORDER BY lieferung_id DESC ';
    }
    if($order_by != '')
    {
